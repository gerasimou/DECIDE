...
int initialiseClient(int portNumber){   
    writeToFile("test.txt", "Initialising client 1\n");
	const char * argv[]={"socket","localhost", "56567"};
    portno = portNumber;//atoi(argv[2]);

    writeToFile("test.txt", "Initialising client 2");
    sockfd = socket(AF_INET, SOCK_STREAM, 0);

    writeToFile("test.txt", "Initialising client 3\n");
    if (sockfd < 0)
        error("ERROR opening socket");


    writeToFile("test.txt", "Initialising client 4\n");
    ...
    writeToFile("test.txt", "Initialising client 5\n");
    memset((char *) &serv_addr, 0, sizeof(serv_addr));

    writeToFile("test.txt", "Initialising client 6\n");
    ...
    writeToFile("test.txt", "Initialising client 7" + to_string(portno) +"\n");
    if (connect(sockfd,(struct sockaddr *)&serv_addr,sizeof(serv_addr)) < 0)
        error("ERROR connecting");

    writeToFile("test.txt", "Initialising client 8\n");
    return (1);
}
